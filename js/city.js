...
$(function(){
    ...
    for (let index = 0; index < province.length; index++) {
        $("#addr-input").append("<option value="+province[index]+">"+province[index]+"</option>");
    };
    //获取option对应的索引
    function getIndex(){
        for (let index = 0; index < province.length; index++) {
            if (province[index]==$("#addr-input option:selected").val()) {
                return index;
            }
        }
    };
    ...
    //根据获取的索引，找到对应的二级城市数据，并循环渲染到页面
    function addCity(){
        var index = getIndex();
        $("#addr-input1").empty();
        for (let i = 0; i < city[index].length; i++) {
            $("#addr-input1").append("<option value="+city[index][i]+">"+city[index][i]+"</option>");
        }
    }
})